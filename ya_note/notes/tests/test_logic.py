...
class TestNotesCreation(TestCase):
    ...
    def test_authenticated_user_can_create_note(self):
        ...
        self.assertEqual(new_note.title, self.form_data[
            'title']), "Заголовок новой заметки не совпадает с формой"
        self.assertEqual(new_note.text, self.form_data[
            'text']), "Текст новой заметки не совпадает с формой"
        self.assertEqual(new_note.slug, self.form_data[
    ...
    def test_cannot_create_note_with_duplicate_slug(self):
        ...
        self.assertEqual(response.status_code,
                         HTTPStatus.OK), "Запрос не вернул код 200"
        note_count = Note.objects.count()
        self.assertEqual(note_count,
                         note_count_before), "Заметка создана"
    ...
    def test_slug_is_generated_if_not_provided(self):
        ...
        response = self.auth_client.post(self.url,
                                         data=form_data)
        self.assertEqual(response.status_code,
                         HTTPStatus.FOUND), "Запрос не вернул код 302"
        note = Note.objects.latest('id')
        self.assertEqual(note.slug, slugify(
            form_data['title'])), "Slug не сгенерирован"
...
class TestNoteEditDelete(TestCase):
    ...
    def test_author_can_edit_own_note(self):
        ...
        self.assertEqual(self.note.title,
                         self.NEW_NOTE_TITLE), "Заголовок заметки не обновлён"
        self.assertEqual(self.note.slug,
    ...
    def test_reader_cannot_edit_others_note(self):
        ...
        response = self.reader_client.post(self.edit_url,
                                           data=self.form_data)
        self.assertEqual(response.status_code,
                         HTTPStatus.NOT_FOUND), "Страница не вернула код 404"
        self.note.refresh_from_db()
        self.assertEqual(self.note.text,
                         self.NOTE_TEXT), "Текст заметки изменён"
        self.assertEqual(self.note.title,
                         self.NOTE_TITLE), "Заголовок заметки изменён"
        self.assertEqual(self.note.slug,
    ...
    def test_reader_cannot_delete_others_note(self):
        ...
        self.assertEqual(response.status_code,
                         HTTPStatus.NOT_FOUND), "Страница не вернула код 404"
        self.assertEqual(Note.objects.count(),
                         note_count_before), "Заметка удалена"
