...
def test_news_on_homepage(client, home_page_url, lots_of_news):
    ...
    assert all_news.count() == NEWS_COUNT_ON_HOME_PAGE, "Number of news on the home page is not equal to NEWS_COUNT_ON_HOME_PAGE"
    assert all_timestamps == sorted_timestamps, "News are not sorted by date, the newest news should be at the top of the list"
...
def test_comments_order_on_news_detail(news,
    ...
    sorted_timestamps = sorted(all_timestamps)
    assert all_timestamps == sorted_timestamps, "Comments are not sorted by date, the newest comments should be at the bottom of the list"
